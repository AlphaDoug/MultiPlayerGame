...
#include "FindSessionsCallbackProxyChinese.h"
#include "Online/OnlineSessionNames.h"


//////////////////////////////////////////////////////////////////////////
// UFindSessionsCallbackProxyChinese


UFindSessionsCallbackProxyChinese::UFindSessionsCallbackProxyChinese(const FObjectInitializer& ObjectInitializer)
...
UFindSessionsCallbackProxyChinese* UFindSessionsCallbackProxyChinese::FindSessionsChinese(UObject* WorldContextObject, class APlayerController* PlayerController, int MaxResults, bool bUseLAN, EBPServerPresenceSearchType ServerTypeToSearch, const TArray<FSessionsSearchSetting> &Filters, bool bEmptyServersOnly, bool bNonEmptyServersOnly, bool bSecureServersOnly, bool bSearchLobbies, int MinSlotsAvailable)
{
	UFindSessionsCallbackProxyChinese* Proxy = NewObject<UFindSessionsCallbackProxyChinese>();
...
void UFindSessionsCallbackProxyChinese::Activate()
{
	FOnlineSubsystemBPCallHelperChinese Helper(TEXT("查找会话"), GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull));
...
void UFindSessionsCallbackProxyChinese::OnCompleted(bool bSuccess)
{
	FOnlineSubsystemBPCallHelperChinese Helper(TEXT("查找会话回调"), GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull));
	Helper.QueryIDFromPlayerController(PlayerControllerWeakPtr.Get());
	...
	if (bSuccess)
	{
		if (bIsOnSecondSearch)
		{
			if (SearchObjectDedicated.IsValid())
			{
				...
				{
					FString ResultText = FString::Printf(TEXT("找到一个会话，Ping是 %d"), Result.PingInMs);
...
void UFindSessionsCallbackProxyChinese::FilterSessionResults(const TArray<FBlueprintSessionResult> &SessionResults, const TArray<FSessionsSearchSetting> &Filters, TArray<FBlueprintSessionResult> &FilteredResults)
{
...
bool UFindSessionsCallbackProxyChinese::CompareVariants(const FVariantData &A, const FVariantData &B, EOnlineComparisonOpRedux Comparator)
{
